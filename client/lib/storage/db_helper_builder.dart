...
class DBHelperBuilder extends SingleChildStatelessWidget {
  ...
  Future<bool> _loadingHelper(BuildContext context) {
    print('VALENCE: DBHelperBuilder._loadingHelper processing...');
    final helper = context.read<DBHelperViewModel>();
    if (helper.mounted && !helper.inited) {
      return helper.init().then((v) {
        print('VALENCE: DBHelperBuilder._loadingHelper done: $v');
        return v;
      }).catchError((e, s) {
        print('VALENCE: DBHelperBuilder._loadingHelper error: $e');
        print('VALENCE: stack: $s');
        return false;
      });
    }
    print('VALENCE: DBHelperBuilder._loadingHelper already inited: ${helper.inited}');
